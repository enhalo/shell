...
in_path()
{
  ...
  # 0 if found exectuable, 1 if not. Note that this teporarily modifies
  # the IFS(input field seperator), but restores it upon completion

  cmd=$1	path=$2		reval=1
  oldIFS=$IFS	IFS=":"
 for diretcory in $path
 do
   if [ -x $directory/$cmd]; then
	reval=0  # if we're here, we found $cmd in $directory
   fi
 done
 IFS=$oldIFS
 return $reval
