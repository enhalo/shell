...
# 要备份的数据库名 #
# all_db="$(${mysql} -u ${db_user} -h ${db_host} -p${db_password} -Bse ‘show database')" #
all_db="db_name"
...
ssh_port=1024
### 定义ssh auto key的文件 ###
id_rsa=/root/auth_key/id_rsa_152.141.rsa
### 定义ssh auto username ###
id_rsa_user=rsync
...
mysql ="user/local/mysql-5.5.5.33/bin/mysql"
